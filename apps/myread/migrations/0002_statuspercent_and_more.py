# Generated by Django 5.0.6 on 2024-06-27 12:12

import django.contrib.postgres.fields.ranges
import django.contrib.postgres.validators
from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('myread', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='StatusPercent',
            fields=[
                ('id', models.PositiveSmallIntegerField(primary_key=True, serialize=False)),
                ('percentage_read_range', django.contrib.postgres.fields.ranges.IntegerRangeField(blank=True, null=True, validators=[django.contrib.postgres.validators.RangeMinValueValidator(0), django.contrib.postgres.validators.RangeMaxValueValidator(101)])),
                ('read_status', models.CharField(choices=[('pending', 'Pending'), ('reading', 'Reeading'), ('done', 'Done')], default='pending', max_length=10)),
            ],
        ),
        migrations.AddConstraint(
            model_name='statuspercent',
            constraint=models.CheckConstraint(check=models.Q(('read_status__in', ['pending', 'reading', 'done'])), name='myread_statuspercent_read_status_check'),
        ),
    ]
